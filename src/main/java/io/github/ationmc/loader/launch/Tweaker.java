...
package io.github.ationmc.loader.launch;

import io.github.ationmc.loader.mod.ModInfo;
...
public class Tweaker implements ITweaker {
    ...
    public void injectIntoClassLoader(LaunchClassLoader classLoader) {
        ...
        try {
                ...
                public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                    if (file.toFile().getName().endsWith(".jar")) {
                        try {
                            ...
                            if (jarFile.getEntry("ation.mod.json") != null) {
                                classLoader.addURL(file.toUri().toURL());
                            } else {
                                LogWrapper.log("AtionLoader", Level.WARN, "Ignore %s it is not a ation mod or ation.mod.json not found", file.toFile().getName());
                            }
                        } catch (Throwable throwable) {
                            LogWrapper.log("AtionLoader", Level.ERROR, throwable, "File %s fail to load", file.toFile().getName());
                        }
                    }
            ...
            Enumeration<URL> resources = classLoader.getResources("ation.mod.json");
            while (resources.hasMoreElements()) {
                ...
                LogWrapper.log("AtionLoader", Level.INFO, "%s | %s | %s | %s", modInfo.getName(), modInfo.getAuthor(), modInfo.getVersion(), modInfo.getDescription());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
